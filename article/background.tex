...
It is one of the most prominent neuroimaging tool for many applications, such as drug discovery \citep{borsook_role_2006} and neuromodeling \citep{friston_dynamic_2003}.
...
To solve this issue, scans need to be remapped to a reference frame via registration \citep{maintz_overview_nodate, sotiras_deformable_2013}.
As reported by \cite{ioanas_optimized_2019}, the legacy approach for mouse-brain image registration is to modify the data in order to conform to pre-existing functions, designed and optimized for human brain imaging.
...
In recent years it has been shown that convolutional neural networks give the best results for semantic image segmentation in terms of precision and flexibility \citep{geng_survey_2018}.
Especially the U-Net architecture from \citep{ronneberger_u-net:_2015} is to this day one of the most popular in the field of biomedical image segmentation.
...
\cite{imperfect_datasets} show that deep learning methods can indeed show satisfiable results when trained with imperfect training data.
...
We provide the methods as a free and open source softare (FOSS) package \citep[MLEBE]{mlebe} as well as the functions needed for the data analysis in this article as a RepSeP document \citep{repsep}.
